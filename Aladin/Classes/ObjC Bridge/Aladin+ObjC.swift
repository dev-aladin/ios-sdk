...
extension Aladin {
    ...
    public func objc_lookupProfile(username: String, zoneFileLookupURL: URL = URL(string: AladinConstants.NameLookupEndpoint)!, completion: @escaping (ObjCProfile?, Error?) -> ()) {
    ...
    /**
     Generates an authentication request and redirects the user to the Aladin browser to approve the sign in request. redirectURI is the location to which the identity provider will redirect the user after the user approves sign in.
