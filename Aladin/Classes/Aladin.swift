...
public enum AladinConstants {
    ...
    public static let NameLookupEndpoint = "\(DefaultCoreAPIURL)/v1/names/"
    public static let AuthProtocolVersion = "1.1.0"
    public static let DefaultGaiaHubURL = "https://todo-dapp.s3.ap-south-1.amazonaws.com"
    public static let ProfileUserDefaultLabel = "ALADIN_PROFILE_LABEL"
    public static let GaiaHubConfigUserDefaultLabel = "GAIA_HUB_CONFIG"
    public static let AppOriginUserDefaultLabel = "ALADIN_APP_ORIGIN"
}

/**
 A class that contains the native swift implementations of Aladin.js methods and Aladin network operations.
 */
@objc open class Aladin: NSObject, ASWebAuthenticationPresentationContextProviding {

    /**
     A shared instance of Aladin that exists for the lifetime of your app. Use this instance instead of creating your own.
     */
    @objc public static let shared = Aladin()
    
    /**
     Use the latest, beta version of the Aladin browser for authentication.
    */
    ...
    /**
     Generates an authentication request and redirects the user to the Aladin browser to approve the sign in request.
    ...
    public func signIn(redirectURI: URL,
        ...
        var urlComps = URLComponents(string: AladinConstants.BrowserWebAppAuthEndpoint)!
        ...
        let completion: (URL?, Error?) -> () = { url, error in
            ...
            // Cache app origin
            UserDefaults.standard.setValue(appDomain.absoluteString, forKey: AladinConstants.AppOriginUserDefaultLabel)
    ...
    */
    ...
    /**
     ...
     WARNING: This will reset the keychain for all apps using Aladin sign in. Apps that are already signed in will not be affected, but the user will have to reenter their 12 word seed to sign in to any new apps.
     */
    @objc public func promptClearDeviceKeychain() {
        let url = URL(string: "\(AladinConstants.BrowserWebClearAuthEndpoint)")!
        if #available(iOS 12.0, *) {
    ...
    /**
     ...
     - parameter username: The Aladin ID of the profile to look up
     ...
     */
    public func lookupProfile(username: String, zoneFileLookupURL: URL = URL(string: AladinConstants.NameLookupEndpoint)!, completion: @escaping (Profile?, Error?) -> ()) {
    ...
    /**
     Validates the social proofs in a user's profile. Currently supports validation of Facebook, Twitter, GitHub, Instagram, LinkedIn and HackerNews accounts.
     - parameter profile: The Profile to be validated.
     - parameter name: The Aladin name to be validated
     - parameter completion: Callback with an array of validated proof objects, or nil if there was an error.
     */
    ...
    @objc public func getAppBucketUrl(gaiaHubURL: URL, appPrivateKey: String, completion: @escaping (String?) -> ()) {
        guard let privateKey = Aladin.shared.loadUserData()?.privateKey,
    ...
    /**
     ...
     - parameter username: The Aladin ID of the user to look up
    ...
    @objc public func getUserAppFileURL(at path: String,
        ...
        Aladin.shared.lookupProfile(username: username, zoneFileLookupURL: zoneFileLookupURL) { profile, error in
            guard error == nil,
                let profile = profile,
    ...
    /**
     ...
     - parameter path: The path to the file to read
     - parameter decrypt: Try to decrypt the data with the app private key
     - parameter username: The Aladin ID to lookup for multi-player storage
     - parameter app: The app to lookup for multi-player storage. Defaults to current origin.
     - parameter zoneFileLookupURL: The Aladin core endpoint URL to use for zonefile lookup, defaults to "/v1/names/"
     - parameter completion: Callback with retrieved content and any error
     - parameter content: The retrieved content as either Bytes, String, or DecryptedContent
     - parameter error: Error returned by Gaia
     */
    @objc public func getFile(at path: String,
        ...
        guard let appOrigin = app ?? UserDefaults.standard.string(forKey: AladinConstants.AppOriginUserDefaultLabel) else {
            completion(nil, GaiaError.configurationError)
            return
        }
        
        let zoneFileLookupURL = zoneFileLookupURL ?? URL(string: AladinConstants.NameLookupEndpoint)!
        Gaia.getOrSetLocalHubConnection { session, error in
            guard let session = session, error == nil else {
                print("gaia connection error")
                completion(nil, error)
    ...
    @objc public func encryptContent(bytes: Bytes, publicKey: String? = nil) -> String? {
        let key: String?
        if publicKey == nil, let privateKey = Aladin.shared.loadUserData()?.privateKey {
    ...
    public func decryptContent(content: String, privateKey: String? = nil) -> DecryptedValue? {
        guard let key = privateKey ?? Aladin.shared.loadUserData()?.privateKey else {
            return nil
        }
     ...
     */
     @objc public func getNameInfo(fullyQualifiedName: String, completion: @escaping ([String: Any]?, Error?) -> ()) {
        let fetchNameInfo = Promise<[String: Any]>() { resolve, reject in
            let task = URLSession.shared.dataTask(with: URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v1/names/\(fullyQualifiedName)")!) {data, response, error in
                guard error == nil,
                    let data = data,
    ...
    @objc public func getNamespaceInfo(namespaceID: String, completion: @escaping ([String: Any]?, Error?) -> ()) {
        let fetchNamespaceInfo = Promise<[String: Any]>() { resolve, reject in
            let task = URLSession.shared.dataTask(with: URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v1/namespaces/\(namespaceID)")!) {data, response, error in
                guard error == nil,
                    let data = data,
                    let httpResponse = response as? HTTPURLResponse else {
                        reject(GaiaError.requestError)
    ...
    public func getNamesOwned(address: String, completion: @escaping ([String]?, Error?) -> ()) {
        ...
        let fetchNamesOwned = Promise<[String]>() { resolve, reject in
            let url = URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v1/addresses/bitcoin/\(networkAddress)")!
    ...
    public func getNamespaceBurnAddress(namespace: String, completion: @escaping ((String?, Error?) -> ())) {
        let fetchNamespace = Promise<[String: Any]>() { resolve, reject in
            let url = URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v1/namespaces/\(namespace)")!
            let task = URLSession.shared.dataTask(with: url) { data, response, error in
                guard error == nil, let data = data else {
                    reject(GaiaError.requestError)
                    return
    ...
    private func getNamePriceV1(_ fullyQualifiedName: String, completion: @escaping ((units: String, amount: Int)?, Error?) -> ()) {
        let fetchNamePrice = Promise<[String: Any]>() { resolve, reject in
            let url = URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v1/prices/names/\(fullyQualifiedName)")!
            let task = URLSession.shared.dataTask(with: url) { data, response, error in
                guard error == nil, let data = data else {
                    reject(GaiaError.requestError)
                    return
    ...
    private func getNamePriceV2(_ fullyQualifiedName: String, completion: @escaping ((units: String, amount: Int)?, Error?) -> ()) {
        let fetchNamePrice = Promise<[String: Any]>() { resolve, reject in
            let url = URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v2/prices/names/\(fullyQualifiedName)")!
            let task = URLSession.shared.dataTask(with: url) { data, response, error in
                guard error == nil, let data = data else {
                    reject(GaiaError.requestError)
                    return
    ...
    private func getNamespacePriceV1(_ namespaceId: String, completion: @escaping ((units: String, amount: Int)?, Error?) -> ()) {
        let fetchNamespacePrice = Promise<[String: Any]>() { resolve, reject in
            let url = URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v1/prices/namespaces/\(namespaceId)")!
            let task = URLSession.shared.dataTask(with: url) { data, response, error in
                guard error == nil, let data = data else {
                    reject(GaiaError.requestError)
                    return
    ...
    private func getNamespacePriceV2(_ namespaceId: String, completion: @escaping ((units: String, amount: Int)?, Error?) -> ()) {
        let fetchNamespacePrice = Promise<[String: Any]>() { resolve, reject in
            let url = URL(string: "\(AladinConstants.DefaultCoreAPIURL)/v2/prices/namespaces/\(namespaceId)")!
            let task = URLSession.shared.dataTask(with: url) { data, response, error in
                guard error == nil, let data = data else {
                    reject(GaiaError.requestError)
                    return
