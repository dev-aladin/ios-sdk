...
class Encryption {
    ...
    static func decryptPrivateKey(privateKey: String, hexedEncrypted: String) -> String? {
        guard let encryptedData = Data(fromHexEncodedString: hexedEncrypted) else {return nil}
        guard let cipherObjectJSONString = String(data: encryptedData, encoding: .utf8)  else {return nil}
