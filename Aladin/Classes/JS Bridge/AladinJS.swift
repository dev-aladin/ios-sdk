...
class AladinJS {
    ...
    lazy var context: JSContext? = {
        ...
        let bundleURL = frameworkBundle.resourceURL?.appendingPathComponent("Aladin.bundle")
        let resourceBundle = Bundle(url: bundleURL!)
        
        guard let
            JSPath = resourceBundle?.path(forResource: "aladin", ofType: "js") else {
                print("Unable to read resource files.")
                return nil
        }
        
        do {
            let AladinJS = try String(contentsOfFile: JSPath, encoding: String.Encoding.utf8)
            _ = context?.evaluateScript(AladinJS)
