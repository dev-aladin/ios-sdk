...
public class ProfileHelper {
    ...
    public static func fetchCurrentUserProfile(completion: @escaping (Profile?, Error?) -> Void) {
        guard let urlString = Aladin.shared.loadUserData()?.profileURL,
            let url = URL(string: urlString) else {
                completion(nil, NSError(domain: "Aladin", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not load authenticated user data."]))
                return
        }
    ...
    static func storeProfile(profileData: UserData) {
        ...
        if let profile = try? PropertyListEncoder().encode(profileData) {
            UserDefaults.standard.set(profile, forKey: AladinConstants.ProfileUserDefaultLabel)
        }
    }
    
    static func retrieveProfile() -> UserData? {
        if let data = UserDefaults.standard.value(forKey: AladinConstants.ProfileUserDefaultLabel) as? Data {
    ...
    static func clearProfile() {
        UserDefaults.standard.removeObject(forKey: AladinConstants.ProfileUserDefaultLabel)
    }
    ...
    static func resolveZoneFileToProfile(zoneFile: String, publicKeyOrAddress: String, completion: @escaping (Profile?) -> ()) {
        ...
        guard let zoneFile = AladinJS().parseZoneFile(zoneFile: zoneFile),
