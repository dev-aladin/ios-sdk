...
class Gaia {
    ...
    static func setLocalGaiaHubConnection(callback: @escaping (GaiaHubSession?, GaiaError?) -> Void) {
        ...
        let hubURL = userData?.hubURL ?? AladinConstants.DefaultGaiaHubURL
    ...
    private static func saveConfig(_ config: GaiaConfig) {
        ...
        if let config = try? PropertyListEncoder().encode(config) {
            UserDefaults.standard.set(config, forKey: AladinConstants.GaiaHubConfigUserDefaultLabel)
        }
    }
    
    static func resetConfig() {
        UserDefaults.standard.removeObject(forKey: AladinConstants.GaiaHubConfigUserDefaultLabel)
    }
